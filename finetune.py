...
parser.add_argument(
    ...
    type=str,
    metavar="PATH",
    help="Load a checkpoint and fine-tune only the classification head (default: none)",
)
parser.add_argument(
    "--resume",
    default="",
...
def main():
    ...
    if args.finetune_checkpoint and (args.resume or args.initial_checkpoint):
        _logger.warning(
            "Both finetune-checkpoint and resume/initial-checkpoint were specified. "
            "finetune-checkpoint will take precedence."
        )
    ...
    if args.finetune_checkpoint:
        if args.local_rank == 0:
            _logger.info(f"Loading checkpoint for fine-tuning from {args.finetune_checkpoint}")
        ...
        if isinstance(checkpoint, dict) and "state_dict" in checkpoint:
            ...
            model.load_state_dict(state_dict, strict=False)
            if args.local_rank == 0:
                _logger.info(f"Successfully loaded weights from checkpoint {args.finetune_checkpoint}")
        else:
        ...
        for name, param in model.named_parameters():
            if not (name.startswith('head_lif') or name.startswith('head.')):
                param.requires_grad = False
                if args.local_rank == 0 and name.endswith('.weight'):
                    _logger.info(f"Freezing layer: {name}")
        
        if args.local_rank == 0:
            # Count trainable parameters
            trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
            total_params = sum(p.numel() for p in model.parameters())
            _logger.info(f"Fine-tuning only the classification head: {trainable_params}/{total_params} parameters will be updated")
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=""):
    ...
    # # Report SDSA statistics if available
    # if hasattr(model, 'report_sdsa_stats'):
    #     model.report_sdsa_stats()
    # elif hasattr(model, 'module') and hasattr(model.module, 'report_sdsa_stats'):
    #     model.module.report_sdsa_stats()
    # else:
    #     _logger.info("Model doesn't have report_sdsa_stats method")

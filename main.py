...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Only run validation on a pre-trained model checkpoint (requires --initial-checkpoint)",
)
parser.add_argument(
    "--auto-skip",
    action="store_true",
    default=False,
    help="Automatically skip attention blocks with low firing rates",
)
parser.add_argument(
    "--skip-threshold",
    type=float,
    default=0.001,
    help="Threshold for skipping attention blocks based on firing rate",
)
parser.add_argument(
    "--log-file",
    type=str,
    default=None,
    help="Path to log file for auto-skipping based on firing rates",
)
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        try:
            ...
            # Optional: limit memory usage to 90% of available memory
            # torch.cuda.set_per_process_memory_fraction(0.9)
        except Exception as e:
            _logger.warning(f"Could not set CUDA memory limits: {str(e)}")
    ...
    # Log auto-skip settings if enabled
    if args.auto_skip and args.local_rank == 0:
        _logger.info(f"Auto-skip enabled with threshold: {args.skip_threshold}")
        if args.log_file:
            _logger.info(f"Auto-skip logs will be written to: {args.log_file}")
        else:
            _logger.warning("Auto-skip enabled but no log file specified. Specify --log-file for better debugging.")
    ...
    try:
        # If validate-only mode is enabled, skip training and run validation directly
        if args.validate_only:
            if not args.initial_checkpoint:
                _logger.error("--validate-only requires --initial-checkpoint to be specified")
                return
                
            _logger.info(f"Validate-only mode: Loading model from {args.initial_checkpoint}")
            ...
            # Load the checkpoint
            checkpoint = torch.load(args.initial_checkpoint, map_location='cuda')
            if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
                state_dict = clean_state_dict(checkpoint['state_dict'])
                model.load_state_dict(state_dict, strict=False)
            else:
            ...
            eval_metrics = validate(
                ...
                log_suffix=' (Validate Only)'
            )
            
            # Log the results
            _logger.info('\nValidation results:')
            _logger.info(f"Top-1 Accuracy: {eval_metrics['top1']:.2f}%")
            _logger.info(f"Top-5 Accuracy: {eval_metrics['top5']:.2f}%")
            
            if args.early_exit and args.dataset in ["cifar10-dvs", "gesture", "imagenet"]:
                _logger.info("Running early exit inference (SEENN-I style)...")
                try:
                    acc, avg_time, time_ratio = early_exit_inference(model, loader_eval, args, output_dir=output_dir)
                    _logger.info(f"[Early Exit] Threshold: {args.exit_threshold}, Avg Time: {avg_time:.2f}, Acc: {acc:.2f}, Portion: {time_ratio}")
                except Exception as e:
                    _logger.error(f"Error during early exit inference: {str(e)}")
                    _logger.error(traceback.format_exc())
            
            # Save the statistics to a separate file
            if output_dir:
                stats_file = os.path.join(output_dir, 'validate_only_stats.json')
                with open(stats_file, 'w') as f:
                    json.dump({
                        'top1': eval_metrics['top1'],
                        'top5': eval_metrics['top5'],
                        'firing_rate': eval_metrics['firing_rate'],
                        'non_zero': eval_metrics['non_zero']
                    }, f, indent=4)
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            try:
                ...
                if "CUDA out of memory" in str(e):
                    _logger.error(f"CUDA OOM at epoch {epoch} during training. Attempting recovery...")
                    torch.cuda.empty_cache()
                    # Try to save checkpoint before potentially crashing
                    if saver is not None and args.rank == 0:
                        try:
                            emergency_path = os.path.join(output_dir, f'emergency-oom-{epoch}.pth.tar')
                            saver.save_checkpoint(epoch, metric=0)
                            _logger.info(f"Emergency checkpoint saved during OOM at {emergency_path}")
                        except Exception as save_err:
                            _logger.error(f"Failed to save emergency checkpoint: {str(save_err)}")
            ...
            try:
                ...
                if "CUDA out of memory" in str(e):
                    _logger.error(f"CUDA OOM at epoch {epoch} during validation. Attempting recovery...")
                    torch.cuda.empty_cache()
                    # Force a simple validation without stats collection if available
                    try:
                        _logger.info("Attempting simplified validation without statistics...")
                        args.collect_stats = False
                        eval_metrics = validate(
                            model, loader_eval, validate_loss_fn, args, output_dir=output_dir, amp_autocast=amp_autocast, log_suffix=" (simplified)"
                        )
                    except Exception:
                        _logger.error("Simplified validation failed, using dummy metrics")
                        # Create dummy metrics to allow training to continue
        ...
        if args.local_rank == 0 and output_dir:
            ...
            if os.path.exists(best_checkpoint):
                ...
                # Log auto-skip settings if enabled for the final evaluation
                if args.auto_skip:
                    _logger.info(f"Auto-skip enabled with threshold: {args.skip_threshold}")
                    if args.log_file:
                        _logger.info(f"Auto-skip logs will be written to: {args.log_file}")
                ...
                if args.early_exit and args.dataset in ["cifar10-dvs", "gesture", "imagenet"]:
                    _logger.info("Running early exit inference (SEENN-I style)...")
                    try:
                        acc, avg_time, time_ratio = early_exit_inference(best_model, loader_eval, args, output_dir=output_dir)
                        _logger.info(f"[Early Exit] Threshold: {args.exit_threshold}, Avg Time: {avg_time:.2f}, Acc: {acc:.2f}, Portion: {time_ratio}")
                    except Exception as e:
                        _logger.error(f"Error during early exit inference: {str(e)}")
                        _logger.error(traceback.format_exc())
...
def validate(model, loader, loss_fn, args, output_dir=None, amp_autocast=suppress, log_suffix=""):
    ...
    # Reduce the number of batches for statistics collection to save memory
    spike_stats_batches = min(5, len(loader))  # Reduced from 10 to 5 to save memory
    
    # Check if we should collect stats (can be disabled in case of OOM)
    collect_stats = getattr(args, 'collect_stats', True)
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                ...
                collect_this_batch = collect_stats and batch_idx < spike_stats_batches
            ...
            del output, loss, acc1, acc5, input, target
            ...
            # Add occasional cache clearing during validation
            if batch_idx % 10 == 0:
                torch.cuda.empty_cache()
...
def early_exit_inference(model, loader, args, output_dir=None):
    """
    ...
    # Add counters for per-class statistics
    class_total = np.zeros(args.num_classes)
    class_correct = np.zeros(args.num_classes)
    class_time_sum = np.zeros(args.num_classes)  # Sum of timesteps for each class
    class_exit_timesteps = [[] for _ in range(args.num_classes)]  # Store all exit timesteps per class
    
    # Add counters for confidence metrics
    confidence_at_t = [[] for _ in range(args.time_steps)]  # Store confidence values at each timestep
    
    # Reduce debug logging for large datasets
    debug_samples = 10 if args.dataset == "imagenet" else 100
    
    # IMPORTANT FIX: Temporarily force model to output temporal data for early exit
    # Store original TET setting
    original_tet = None
    if hasattr(model, 'TET'):
        original_tet = model.TET
        model.TET = True  # Force temporal output
        _logger.info(f"Temporarily set model.TET=True for early exit (was {original_tet})")
    elif hasattr(model, 'module') and hasattr(model.module, 'TET'):
        original_tet = model.module.TET
        model.module.TET = True  # For distributed models
        _logger.info(f"Temporarily set model.module.TET=True for early exit (was {original_tet})")
    ...
    try:
        with torch.no_grad():
            for batch_idx, (input, target) in enumerate(loader):
                ...
                try:
                    ...
                    # Some models might not return firing dict when not asked for
                    output = model(input)
                    if isinstance(output, tuple):
                        output = output[0]
                
                
                # Handle different output shapes based on TET and model type
                if output.dim() == 3:
                    # Output: (T, B, num_classes) - This is what we want for early exit
                    T, B, C = output.shape                  
                    
                    # For each sample in batch, do early exit
                    for b in range(B):
                        class_idx = target[b].item()
                        if class_idx < len(class_total):  # Safety check for class index
                        ...
                        for t in range(T):
                            ...
                            if total < debug_samples:
                                confidence_at_t[t].append(metric_val)
                            
                            # Check if we should exit
                            if (metric_val > args.exit_threshold) or t == T - 1:
                                exit_timestep = t + 1  # Convert to 1-indexed timestep
                                pred = partial_logits.argmax().item()
                                is_correct = int(pred == target[b].item())
                                ...
                                if class_idx < len(class_total):  # Safety check
                                    if is_correct:
                                        class_correct[class_idx] += 1
                                    class_time_sum[class_idx] += exit_timestep
                                    class_exit_timesteps[class_idx].append(exit_timestep)
                                
                                exited = True
                                break
                        
                        # In case no exit happened (should not occur due to t == T-1 condition)
                        if not exited:
                            exit_timestep = T
                            pred = partial_logits.argmax().item()
                            is_correct = int(pred == target[b].item())
                    ...
                    # Handle case where model still doesn't output temporal dimension
                    # This should not happen after forcing TET=True, but just in case
                    _logger.warning(f"Model output is still non-temporal ({output.shape}) even after forcing TET=True")
                    B, C = output.shape
                    # For non-temporal case, we can't do early exit since we don't have temporal information
                    pred = output.argmax(dim=1)
                    correct += (pred == target).sum().item()
                    total += target.size(0)
                    time_vec[0] += target.size(0)  # All samples exit at first timestep
                    
                    # Update per-class statistics for non-temporal case
                    for b in range(B):
                        class_idx = target[b].item()
                        if class_idx < len(class_total):  # Safety check for class index
                            class_total[class_idx] += 1
                            if pred[b].item() == class_idx:
                                class_correct[class_idx] += 1
                            class_time_sum[class_idx] += 1  # Always first timestep
                            class_exit_timesteps[class_idx].append(1)  # Always exit at timestep 1
        ...
        # IMPORTANT: Restore original TET setting
        if original_tet is not None:
            if hasattr(model, 'TET'):
                model.TET = original_tet
                _logger.info(f"Restored model.TET to {original_tet}")
            elif hasattr(model, 'module') and hasattr(model.module, 'TET'):
                model.module.TET = original_tet
                _logger.info(f"Restored model.module.TET to {original_tet}")
    ...
    if any(confidence_at_t[0]):
        _logger.info("\nConfidence statistics for first few samples:")
        for t in range(args.time_steps):
            if confidence_at_t[t]:
                avg_conf = sum(confidence_at_t[t]) / len(confidence_at_t[t])
                min_conf = min(confidence_at_t[t]) if confidence_at_t[t] else 0
                max_conf = max(confidence_at_t[t]) if confidence_at_t[t] else 0
                _logger.info(f"Timestep {t+1}: Avg={avg_conf:.4f}, Min={min_conf:.4f}, Max={max_conf:.4f}")
    
    # Calculate per-class statistics
    class_acc = 100.0 * class_correct / np.maximum(class_total, 1)  # Avoid division by zero
    class_avg_time = class_time_sum / np.maximum(class_total, 1)  # Avoid division by zero
    
    # Calculate std dev of exit timesteps per class
    class_time_std = np.zeros(args.num_classes)
    for i in range(args.num_classes):
        if len(class_exit_timesteps[i]) > 0:
            class_time_std[i] = np.std(class_exit_timesteps[i])
    
    # Log overall statistics
    _logger.info(f"Early Exit: Threshold={args.exit_threshold}, Metric={args.exit_metric}")
    _logger.info(f"Overall Acc={acc:.2f}%, Avg Time={avg_time:.2f}, Portion={time_ratio}")
    
    # Log per-class statistics
    _logger.info("\nPer-class Early Exit Statistics:")
    _logger.info("Class ID\tSamples\tAccuracy\tAvg Timestep\tStd Dev")
    _logger.info("-" * 50)
    
    for i in range(args.num_classes):
        if class_total[i] > 0:
            _logger.info(f"{i}\t{int(class_total[i])}\t{class_acc[i]:.2f}%\t{class_avg_time[i]:.2f}\t{class_time_std[i]:.2f}")
    
    if output_dir:
        # Save detailed statistics to a file 
        stats_file = os.path.join(output_dir, 'early_exit_class_stats.json')
        
        stats_data = {
            'overall': {
                'accuracy': float(acc),
                'avg_timestep': float(avg_time),
                'exit_timestep_ratio': [float(x) for x in time_ratio],
            ...
            'per_class': {
                str(i): {
                    ...
                    'avg_timestep': float(class_avg_time[i]),
                    'std_dev': float(class_time_std[i]),
                    'exit_timesteps': [int(t) for t in class_exit_timesteps[i]] if len(class_exit_timesteps[i]) <= 100 else 
                                        "too many to store"
                } for i in range(args.num_classes) if class_total[i] > 0
            }
        }
        ...
        if any(confidence_at_t[0]):
            ...
            for t in range(args.time_steps):
                if confidence_at_t[t]:
                    confidence_stats[f"timestep_{t+1}"] = {
                        "avg": float(sum(confidence_at_t[t]) / len(confidence_at_t[t])),
                        "min": float(min(confidence_at_t[t])),
                        "max": float(max(confidence_at_t[t])),
                        "values": [float(c) for c in confidence_at_t[t][:20]]  # Store first 20 values
                    }
            stats_data["confidence"] = confidence_stats
        
        with open(stats_file, 'w') as f:
            json.dump(stats_data, f, indent=4)
        _logger.info(f"\nSaved detailed early exit statistics to {stats_file}")
